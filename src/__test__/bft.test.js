...
import BreadthFirstTraversal from '../lib/breadth-first-traversal';

describe('BFT', () => {
  test('Testing breadth-first traversal', () => {
    ...
    const paths = BreadthFirstTraversal(graph, node5, node25);
    console.log(paths); /* eslint-disable-line */
    expect(paths).not.toBeNull();
    expect(paths.has(node25)).toBeTruthy();
    const nonExistentPaths = BreadthFirstTraversal(graph, node5, node100);
    expect(nonExistentPaths).not.toBeNull();
  });
});
